 ...
 * URL が一覧の registry の <base>/<uuid>/….xpi なら、読み込みを止めて about:nora:settings#drop=<uuid>&registry=<name> を開く。
...
export function registerDropLinks(): void {
  ...
  const handler = {
    handleContent(mimetype: string, context: unknown, request: nsIRequest) {
      ...
      const browser = channel.loadInfo.targetBrowsingContext?.top?.embedderElement as { ownerGlobal?: any } | undefined;
      const win = browser?.ownerGlobal ?? Services.wm.getMostRecentWindow("navigator:browser");
      const url = `about:nora:settings#drop=${hit.uuid}&registry=${encodeURIComponent(hit.registry)}`;
